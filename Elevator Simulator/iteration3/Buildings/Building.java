...
public class Building implements ElevatorObserver, FloorObserver {
	...
	// Class to hold the floor requests for the mWaitingFloors with a destination and a direction
	...
	public String toString(){
		...
		for(int floorNumber = mFloors.size(); floorNumber > 0; floorNumber-- ){
			...
			for(int i = 0; i < mFloors.get(floorNumber-1).getWaitingPassengers().size(); i++){
				...
				finalString1 += " " + mFloors.get(floorNumber-1).getWaitingPassengers().get(i).getShortName() +
						mFloors.get(floorNumber-1).getWaitingPassengers().get(i).getId() + "->" + 
				mFloors.get(floorNumber-1).getWaitingPassengers().get(i).getDestination();// +1 after getDest()
			}
			finalString1 += "\n";
		}
		String finalString2 = "";
	...
	public void elevatorWentIdle(Elevator elevator) {
		...
		if(!mWaitingFloors.isEmpty()){
			//elevator.dispatchToFloor(elevator,mFloors.get(mWaitingFloors.remove().mDestination.getNumber() -1),elevator.getCurrentDirection());
            
			Floor floor = mFloors.get(mWaitingFloors.remove().mDestination.getNumber() -1);
			/*
            if(elevator.getCurrentFloor().getNumber() < floor.getNumber()){
                elevator.setCurrentDirection(Elevator.Direction.MOVING_UP);
            }
            else if(elevator.getCurrentFloor().getNumber() > floor.getNumber()){
                elevator.setCurrentDirection(Elevator.Direction.MOVING_DOWN);
            }
            */
            elevator.dispatchToFloor(elevator, floor,floor.getFloorDirection());
	...
	public void directionRequested(Floor floor, Elevator.Direction direction) {
		...
		for(Elevator e : mElevators){
			if(e.canBeDispatchedToFloor(e, floor)){
				e.dispatchToFloor(e, floor, direction);
				idle++;
			}
		}
		if(idle == 0){
