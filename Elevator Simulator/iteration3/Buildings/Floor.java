...
public class Floor implements ElevatorObserver {
	...
	public Elevator.Direction getFloorDirection(){
        ...
        int dest = 0;
        for(Passenger p : this.getWaitingPassengers()){
            dest = p.getDestination();
            break;
        }

        if(dest > this.getNumber()){
            floorDirection = Elevator.Direction.MOVING_UP;
        }
        else if(dest < this.getNumber()){
            floorDirection = Elevator.Direction.MOVING_DOWN;
        }
	...
	public void clearDirection(Elevator.Direction direction) {
        ...
        if((direction == Elevator.Direction.MOVING_UP || direction == Elevator.Direction.NOT_MOVING) && upButton){
            upButton = false;
        }
        else if((direction == Elevator.Direction.MOVING_DOWN || direction == Elevator.Direction.NOT_MOVING)  && downButton){
            downButton = false;
        }
	}
