...
public abstract class Passenger implements FloorObserver, ElevatorObserver {
	...
	public void elevatorArriving(Floor floor, Elevator elevator) {
		...
		if (floor.getWaitingPassengers().contains(this) && mCurrentState == PassengerState.WAITING_ON_FLOOR) {
			...
			if(elevatorDirection == Elevator.Direction.NOT_MOVING || 
					(getDestination() > elevator.getCurrentFloor().getNumber() && elevatorDirection == Elevator.Direction.MOVING_UP)||
					(getDestination() < elevator.getCurrentFloor().getNumber() && elevatorDirection == Elevator.Direction.MOVING_DOWN)) {
	...
	public void elevatorDoorsOpened(Elevator elevator) {
		// TODO: The elevator is arriving at our destination. Remove ourselves from the elevator, and stop observing it.
		// TODO: Does NOT handle any "next" destination...
		...
		// from the floor, and enter the elevator.
			...
			if(willBoardElevator(elevator) && elevator.getCurrentFloor() == passFloor) {
